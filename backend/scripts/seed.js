...
const scales_1 = require("../src/lib/scales");
...
async function main() {
    ...
    for (const scale of scales_1.AUTHORIZED_SCALES) {
    ...
    if (!fictitiousPatient) {
        fictitiousPatient = await prisma.patient.create({
            data: {
                ...
                phone: '(11) 98765-4321',
                notes: (0, crypto_1.encrypt)('Paciente fictício para testes seguros.'),
                status: 'ATIVO',
            },
        });
        await prisma.anamnesis.create({
            data: {
                ...
                dataEncrypted: (0, crypto_1.encryptJSON)({
        ...
        const s1 = await prisma.session.create({
            data: {
                ...
                sessionTime: '14:00',
                status: 'REALIZADA',
                rawNotesEncrypted: (0, crypto_1.encrypt)('Primeira sessão de acolhimento.'),
            },
        });
        await prisma.sessionNote.create({
            data: {
                sessionId: s1.id,
                contentEncrypted: (0, crypto_1.encrypt)(`O paciente compareceu pontualmente ao primeiro atendimento de alinhamento terapêutico, apresentando-se motivado para o processo. Relatou incômodo persistente relacionado à urgência constante e dificuldade para desacelerar ao término do expediente de trabalho.
...
Como combinados, o paciente concordou em registrar situações gatilho e iniciar protocolo de higiene do sono.`),
        ...
        await prisma.medicalRecordEntry.create({
            data: {
                sessionId: s1.id,
                contentEncrypted: (0, crypto_1.encrypt)(`Realizado atendimento psicológico inicial com foco em acolhimento e enquadre terapêutico. Aplicados procedimentos de psicoeducação sobre ansiedade e treino guiado de regulação respiratória. Observada boa receptividade. Combinada continuidade do acompanhamento.`),
                status: 'APROVADO',
            },
        });
