...
const DashboardStats: React.FC = () => {
  ...
  const statsList = [
    { title: 'Total Pacientes', value: stats?.totalPatients || 0, icon: <Users className="text-blue-600" />, trend: '+12% este mês', color: 'bg-blue-50' },
    { title: 'Forms Respondidos', value: stats?.formsAnswered || 0, icon: <FileCheck className="text-emerald-600" />, trend: '+5% hoje', color: 'bg-emerald-50' },
    { title: 'Pendentes', value: stats?.pendingForms || 0, icon: <Clock className="text-amber-600" />, trend: '-2 desde ontem', color: 'bg-amber-50' },
    { title: 'Taxa de Retorno', value: stats?.returnRate || '0%', icon: <TrendingUp className="text-purple-600" />, trend: '+2.4% vs média', color: 'bg-purple-50' },
  ];
